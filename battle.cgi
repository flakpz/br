...
sub WINCHG {
	...
	if (length($msg2) > 100) { $log = ($log . "살해시 대사의 글자수가 제한을 넘었습니다. \(한글 32글자까지\)<br>살해시 대사는 변경되지 않았습니다.<br>"); }
	else { $msg = $msg2; $checkwinchg = 1; }
	if (length($dmes2) > 100) { $log = ($log . "유언의 글자수가 제한을 넘었습니다. \(한글 32글자까지\)<br>유언은 변경되지 않았습니다.<br>"); }
	else { $dmes = $dmes2; $checkwinchg = 1; }
	if (length($com2) > 100) { $log = ($log . "코멘트의 글자수가 제한을 넘었습니다. \(한글 32글자까지\)<br>코멘트는 변경되지 않았습니다.<br>"); }
...
sub DEATHGET {
	...
	if ($w_death =~ /참살/) {
		...
		elsif ($w_com == 2) {$log = ($log . "폭탄에 공격당한 것일까, 머리와 오른쪽 팔만 남아있다...<br>\n") ;}
		elsif ($w_com == 3) {$log = ($log . "폭탄에 날아간 것일까, 머리가 반정도 사라져 안이 들여다 보인다...<br>\n") ;}
		elsif ($w_com == 4) {$log = ($log . "폭발로 날아간 한쪽 팔이 5m정도 앞에 뒹굴고 있다...<br>\n") ;}
		elsif ($w_com == 5) {$log = ($log . "시체라기보다, 살덩어리구나...<br>\n") ;}
		else {$log = ($log . "목과 손이 보이지않네... 폭발으로 날아간 것일까...<br>\n") ;}
	}elsif ($w_death =~ /박살/) {
