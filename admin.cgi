...
sub MAIN {
    ...
    push(@log,"관리용 패스워드&nbsp;&nbsp;<INPUT size=\"16\" type=\"password\" name=\"Password\" maxlength=\"16\">\n");
    push(@log,"　<INPUT type=\"submit\" name=\"Enter\" value=\"확인\">\n");
    push(@log,"</FORM>\n");

    &HEADER ;
    print "</b></center>\n" ;
    print "■ BATTLE ROYALE 관리자 모드 ■</TD></TR><TR><TD valign=top><BR>\n";
    print @log;
    &FOOTER;
}
#==================#
...
sub DATARESET {
    ...
    $dhour = $hour + 4;
    if ( $dhour >= 24 ) {
         $dhour = $dhour - 24; $dmday + 1;
     } 
    $areadata[0] = ($year . "," . $month . "," . $dmday . "," . $dhour . ",0\n") ;   #구역 추가 시간
    $areadata[1] = "1,0,\n" ; #금지 지역, 해킹 프래그
...
sub USERLIST {
    ...
    &HEADER ;
    print "</b></center>\n" ;
    print "■ BATTLE ROYALE 관리자 모드 - 생존자 일람 ■</TD></TR><TR><TD valign=top><BR><center>\n";
    print @log;
    &FOOTER;
